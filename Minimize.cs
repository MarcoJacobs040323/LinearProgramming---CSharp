...
internal class Minimize : Pivot
{
    ...
    {
        { 3.5, 1, 0, 2, 2, 1, 0.5, 20 },
        {  1.833333333, 1.333333333, 1,  2.333333333, 1.666666667, 1.666666667,0.166666667, -5},
    });//Create a testTable array using Santa's values for testing purposes
    ...
    public List<Table> Duality()
    {
        tables.Clear();
        tables.Add(table2);
        int pivotRow = findPivotRow(table2);
        while (pivotRow != -1)//While the table is not optimal
        {
            int pivotColumn = findPivotColumn(table2, pivotRow);
            if (pivotColumn == -1)//If there is no valid pivot column, the solution is unbounded
            {
                break;
            }
            table2 = findNextTable(table2, pivotRow, pivotColumn);
            tables.Add(table2);
            pivotRow = findPivotRow(table2);
        }
        printTables();
        return tables;
    }

    public List<Table> ChangeConstraint()
    {
        tables.Clear();
        tables.Add(table3);
        int pivotRow = findPivotRow(table3);
        while (pivotRow != -1)//While the table is not optimal
        {
            int pivotColumn = findPivotColumn(table3, pivotRow);
            if (pivotColumn == -1)//If there is no valid pivot column, the solution is unbounded
            {
                break;
            }
            table3 = findNextTable(table3, pivotRow, pivotColumn);
            tables.Add(table3);
            pivotRow = findPivotRow(table3);
        }
        printTables();
        return tables;
    }
